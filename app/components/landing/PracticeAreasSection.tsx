  ...
  return (
    <section id="services" className="mx-auto w-full max-w-7xl scroll-mt-24 px-4 py-16 sm:px-6 lg:px-8">
      <div className="mb-10 max-w-2xl">
        <h2 className="text-3xl font-bold tracking-tight text-red-950">{content.title}</h2>
        <p className="mt-3 text-red-900/80">{content.description}</p>
      </div>
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {content.items.map((area) => (
          ...
          >
            <h3 className="text-xl font-semibold text-red-950">{area.title}</h3>
            <p className="mt-3 text-red-900/80">{area.description}</p>
          </article>
        ))}
      </div>
    </section>
